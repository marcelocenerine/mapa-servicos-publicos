	...
	function getLocation() {
		var latdefault = -23.50000000;
		var longdefault = -47.500000;
		initialize(latdefault, longdefault, 13, 2);
		if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(
				function (position) {
					initialize(position.coords.latitude, position.coords.longitude, 16, 1);
				},
				function (erro) {
					initialize(latdefault, longdefault, 13, 2);
				}
	...
	function initialize(latitude, longitude, zoom, enderecodesc) {
		...
		$.getJSON('rest/api/servicos/lng/'+longitude+'/lat/'+latitude+'/categorias/INSS,RFB,ASS_SOCIAL,UBS,ENS_BASICO', function(pontos) {
